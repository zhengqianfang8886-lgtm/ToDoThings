...
import { 
  ...
  Settings, Moon, Sun, CheckSquare, Inbox, Check, BookOpen, Copy, Loader2
} from 'lucide-react';
...
export default function App() {
  ...
  // Update tag colors dynamically when theme switches
  ...
  const handleSidebarClick = (type: 'inbox' | 'today' | 'logbook' | 'templates') => {
    ...
    if (type === 'templates') {
      engine.setActiveView(ViewMode.TEMPLATES);
    } else {
      if (engine.activeView === ViewMode.TEMPLATES) {
        engine.setActiveView(ViewMode.LIST);
      }
    }
  };

  const contextTasks = useMemo(() => {
    return engine.tasks.filter(tk => {
      if (tk.parentId) return false;
      const matchesSearch = tk.title.toLowerCase().includes(engine.searchQuery.toLowerCase());
      const matchesTag = !engine.selectedTag || tk.tags.includes(engine.selectedTag);
      const matchesToday = !engine.isTodayView || (tk.dueDate ? isToday(new Date(tk.dueDate)) : false);
      return matchesSearch && matchesTag && matchesToday;
    });
  }, [engine.tasks, engine.searchQuery, engine.selectedTag, engine.isTodayView]);
    ...
    <div className="flex h-screen overflow-hidden bg-things-lightBg dark:bg-things-bg font-sans selection:bg-things-accent selection:text-white transition-colors duration-500">
      ...
      <AnimatePresence>
        {!engine.isLoaded && (
          <motion.div 
            initial={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
            className="fixed inset-0 z-[500] bg-things-lightBg dark:bg-things-bg flex flex-col items-center justify-center space-y-4"
          >
      ...
      <aside className="w-64 border-r border-things-lightBorder dark:border-things-border px-3 py-6 flex flex-col bg-things-lightSidebar dark:bg-things-sidebar transition-all duration-500">
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-things-accent rounded-apple-md flex items-center justify-center text-white shadow-apple-sm">
              <CheckSquare size={18} />
            </div>
          ...
          <button onClick={() => setIsSettingsOpen(true)} className="p-2 hover:bg-white dark:hover:bg-things-hover rounded-apple-sm transition-all border border-transparent hover:border-things-lightBorder dark:hover:border-things-border">
            <Settings size={14} className="text-things-subtle" />
          </button>
        </div>
        ...
        <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-1">
          <nav className="space-y-0.5">
            <SidebarItem 
              ...
              active={!engine.selectedTag && !engine.isTodayView && !engine.isLogbookView && engine.activeView !== ViewMode.TEMPLATES} 
              count={inboxCount} 
              progress={(engine.tasks.filter(tk => !tk.parentId && tk.completed).length / Math.max(1, engine.tasks.filter(tk => !tk.parentId).length)) * 100} 
            />
            <SidebarItem 
              icon={CalendarIcon} 
              label={t.today} 
              onClick={() => handleSidebarClick('today')} 
              active={engine.isTodayView} 
              count={todayCount} 
              progress={(engine.tasks.filter(tk => tk.dueDate && isToday(new Date(tk.dueDate)) && !tk.parentId && tk.completed).length / Math.max(1, engine.tasks.filter(tk => tk.dueDate && isToday(new Date(tk.dueDate)) && !tk.parentId).length)) * 100} 
            />
            <SidebarItem 
              icon={BookOpen} 
              label={t.logbook} 
              onClick={() => handleSidebarClick('logbook')} 
              active={engine.isLogbookView} 
              count={logbookCount} 
            />
            <SidebarItem 
              icon={Copy} 
              ...
              active={engine.activeView === ViewMode.TEMPLATES} 
              count={engine.templates.length} 
            />
          </nav>
          ...
          <section>
            <div className="flex items-center justify-between mb-2 px-3">
              ...
              <button onClick={() => setIsTagModalOpen(true)} className="p-1 hover:bg-black/5 dark:hover:bg-things-hover rounded text-things-subtle transition-colors">
                <Settings size={10}/>
              </button>
            </div>
            <div className="space-y-0.5">
              {themedTags.map(tag => {
                const tagTasks = engine.tasks.filter(tk => tk.tags.includes(tag.id) && !tk.parentId);
                const count = tagTasks.filter(tk => !tk.completed).length;
                const progress = tagTasks.length > 0 ? (tagTasks.filter(tk => tk.completed).length / tagTasks.length) * 100 : 0;
                return (
                  <SidebarItem 
                    key={tag.id}
                    label={tag.name}
                    customIconColor={tag.color}
                    onClick={() => { 
                      engine.setSelectedTag(engine.selectedTag === tag.id ? null : tag.id); 
                      engine.setIsTodayView(false); 
                      engine.setIsLogbookView(false); 
                      engine.setActiveView(ViewMode.LIST); 
                    }}
                    active={engine.selectedTag === tag.id}
                    count={count}
                    progress={progress}
                  />
                );
        ...
        <div className="mt-4 pt-4 border-t border-things-border/30 dark:border-things-border/10 space-y-3 px-1">
          <button 
            onClick={() => engine.setIsDarkMode(!engine.isDarkMode)} 
            className="w-full flex items-center justify-center space-x-2 py-2 text-[11px] font-bold uppercase text-things-secondary hover:text-things-accent rounded-apple-md hover:bg-black/5 dark:hover:bg-things-hover transition-all"
          >
            {engine.isDarkMode ? <Sun size={14} /> : <Moon size={14} />}
            <span>{t.toggleTheme}</span>
          </button>
          <button 
            onClick={() => { engine.addTask(t.newTask); engine.setIsLogbookView(false); }} 
            className="w-full flex items-center justify-center space-x-2 py-3 bg-things-accent text-white rounded-apple-md font-bold text-[13px] shadow-apple-md hover:brightness-110 active:scale-[0.98] transition-all"
          >
            <Plus size={18} />
            <span>{t.newTask}</span>
          </button>
      ...
      <main className="flex-1 flex flex-col bg-things-lightBg dark:bg-things-bg transition-colors duration-500">
        <header className="px-8 py-4 flex items-center justify-between border-b border-things-lightBorder/30 dark:border-things-border/10 bg-things-lightBg/80 dark:bg-things-bg/80 backdrop-blur-xl z-20">
          <div className="flex items-center bg-things-lightSidebar dark:bg-things-sidebar px-4 py-2 rounded-apple-md flex-1 max-w-sm border border-things-lightBorder/50 dark:border-things-border shadow-apple-sm transition-all focus-within:ring-2 focus-within:ring-things-accent/10 focus-within:border-things-accent/40">
            ...
            <input 
              ...
              className="bg-transparent border-none focus:outline-none ml-3 text-[13px] w-full font-medium text-things-lightText dark:text-things-text"
          ...
          <div className="flex bg-things-lightSidebar dark:bg-things-sidebar p-1 rounded-apple-lg border border-things-lightBorder/50 dark:border-things-border ml-6 shadow-apple-sm transition-all">
            {[
              { id: ViewMode.LIST, icon: List },
              { id: ViewMode.KANBAN, icon: LayoutGrid },
              { id: ViewMode.CALENDAR, icon: CalendarIcon },
              { id: ViewMode.STATS, icon: BarChart3 }
            ].map((v) => (
              <button 
                key={v.id} 
                onClick={() => engine.setActiveView(v.id)} 
                disabled={engine.activeView === ViewMode.TEMPLATES}
                className={`p-2.5 rounded-apple-md transition-all duration-300 ${engine.activeView === v.id ? 'bg-white dark:bg-things-card shadow-apple-sm text-things-accent scale-105' : 'text-things-subtle hover:text-things-lightText dark:hover:text-things-text disabled:opacity-20'}`}
              >
                <v.icon size={16} />
              </button>
            ))}
        ...
        <div className="h-0.5 w-full bg-things-border/10 relative overflow-hidden">
          <motion.div 
            animate={{ left: `${contextProgress - 100}%` }} 
            transition={{ type: 'spring', damping: 25, stiffness: 120 }}
            className={`absolute inset-y-0 w-full ${getProgressBgClass(contextProgress)}`} 
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pt-8">
          <AnimatePresence mode="wait">
            <motion.div 
              key={engine.activeView + (engine.isLogbookView ? '-log' : '')} 
            ...
            >
              {engine.activeView === ViewMode.LIST && (
                <div className="space-y-6">
                  <div className="flex items-end justify-between mb-10">
                    <div>
                      <h2 className="text-[34px] font-black tracking-tighter leading-none text-things-lightText dark:text-things-text">
                        {engine.isLogbookView ? t.logbook : (engine.isTodayView ? t.today : (engine.selectedTag ? themedTags.find(tg => tg.id === engine.selectedTag)?.name : t.inbox))}
                      </h2>
                  ...
                  <Reorder.Group axis="y" values={engine.filteredTasks} onReorder={(newOrder) => {
                    const others = engine.tasks.filter(tk => tk.parentId || !engine.filteredTasks.find(f => f.id === tk.id));
                    engine.setTasks([...newOrder, ...others]);
                    ...
                    <AnimatePresence initial={false}>
                      {engine.filteredTasks.map(task => (
                        <Reorder.Item key={task.id} value={task}>
                          <TaskCard 
                            task={task} 
                            allTasks={engine.tasks} 
                            tags={themedTags} 
                            language={engine.settings.language}
                            onToggle={engine.toggleTask} 
                            onToggleTimer={engine.toggleTimer}
                            onDelete={engine.deleteTask} 
                            onEdit={setEditingTask} 
                            onUpdateTask={engine.updateTask} 
                            onAddSub={(title, pid) => engine.addTask(title, pid)} 
                          />
                        </Reorder.Item>
                      ))}
                    </AnimatePresence>
                    {engine.filteredTasks.length === 0 && (
                      <div className="py-24 text-center opacity-20 flex flex-col items-center">
                        <CheckSquare size={54} strokeWidth={1} className="mb-6 text-things-secondary" />
                        <p className="font-black uppercase text-[11px] tracking-[0.2em] text-things-secondary">{t.frequencyClear}</p>
                      </div>
                    )}
                  </Reorder.Group>
                </div>
              ...
              {engine.activeView === ViewMode.KANBAN && <KanbanView tasks={engine.filteredTasks} language={engine.settings.language} onToggle={engine.toggleTask} onDelete={engine.deleteTask} />}
              ...
              {engine.activeView === ViewMode.TEMPLATES && (
                <TemplatesView 
                  ...
                  tags={themedTags} 
                  language={engine.settings.language}
                  onAdd={engine.addTemplate} 
                  onUpdate={engine.updateTemplate} 
                  onDelete={engine.deleteTemplate} 
                  onUse={engine.useTemplate} 
                />
              )}
            </motion.div>
          </AnimatePresence>
